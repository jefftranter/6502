...
; THE FOLLOWING IS NOT USED BY THE OS. MAY BE USED BY BASIC
        LDA LAMB       GET LOW ADDRESS OF MENORY BLOCK
...
; THERE IS NO SEEK ERROR. ALSO $F9 WILL BE INITIALIZEO TO 5 INSTEAD
...
;
        ...
        STA SCTBYP      SET SECTORS BYPASSED TO 0
...
RDCONT  LDA #$01
        ...
        BEQ *-3         ($292B) IF NOT, TRY AGAIN
        LDA ACIAIO      GET BYTE FROM ACIA
        BVS SETFF       PARITY ERROR, RETURN
        BCC *+6         ($293B) CARRY CLEAR, THIS IS A READ
        CMP (MEMLO),Y   COMPARE TO BYTE IN MEMOHY
        BNE SETFF       IF NOT THE SAME, ERROR
        STA (MEMLO),Y   STORE BYTE IN MEMORY
        INY             BUMP THE INDEX
        BNE HDCONT      IF MORE THIS PAGE, CONTINUE
        INC MEMHI       SET ADDRESS FOR NEXT PAGE
        DEX             DROP PAGE COUNT
        BNE HDCONT      IF ANOTHER PAGE, CONTINUE
...
RTYRD   LDA #$00        DENOTES READ
        JSR RDCDSK      READ SECTOR INTO NEMORY
        BCC DKRDRY+3    ($297A) IF FAULT OCCURED, RETRY
        RTS
;
; DKRDRY : DISK READ RETRY
;
DKRDRY  DEC MEMHI       HESET MEMORY ADDRESS
        INX
        CPX PGCNT
...
;                       STORE SECTOR LENGTH IN $FA (PAGES)
        DEX
        BNE *-6         ($29A1) BACK FOR SECOND BYTE
        INC SCTBYP      BUMP SECTORS BYPASSED
        TAY             SECTOR LENGTH IN PAGES
        JSR DKBTCI      GET ANOTHER BYTE FROM DISK
        DEX
        BNE *-4         ($29AC) IF NOT END OF PAGE, CONTINUE
        DEY
        BNE *-7         ($29AC) IF MORE PAGES TO GO, CONTINUE
        RTS
;
...
DIRCNT  TAX             PUT TRACK NUMBER IN X
        ...
        BCS *-9         ($2AIB) IF WE DIDN'T HIT THE INDEX
;                       HOLE, TRY AGAIN
...
;                       PRESENT DISK DRIVE
        ...
        JMP OS65D3      LOOP NACK FOR ANOTHER COMMAND
;
...
;
D9      LDA #$00
        STA DK8T9+4     ($28B4) CHANGE DKBT9 ROUTINE
        RTS
;
...
; GO : GO COMMAND
;
        JSR BLDHEX      GET HIGH ORDEH ADDRESS
        STA GOADR+2     ($2B54) SAVE IT
        JSR BLDHEX      GET LOW ORDER ADDRESS
        STA GOADR+l     ($2B53) SAVE IT
        JMP **          GO TD ADDRESS ENTERED
;
...
;                       OTHERWISE, DO ONE TRACK
        ...
        CMP #'Y'
        BNE LOAD-l      ($2BA6) IF NOT 'Y' THEN RETURN
...
MEM     LDX #$00        SET OFFSET FOR INPUT ADDRESS
        JSR *+8         ($2BD0) GET FROM BUFFER AND SAVE IT
...
;
        ...
        JMP *-10        ($2BED) LOOP BACK & ,WRITE THIS TRACK
        JMP UNLDHD      UNLOAD HEAD AND RETURN
;
; RET : RESTART COMMAND
;
...
;
BUFBYT  LDY #BUFOFS     GET OFFSET INTO BUFFER
;                       MORE SELF MODIFYING CODE
        LDA (OSIBAD),Y  LOAD BYTE
        CMP #$0D        CHECK FOR 'CR'
        BEQ *+9         ($2CF3) IF SO WE ARE DONE
...
SWAPLP  LDA STACK,X     GET BYTE FRDM PAGE 1
...
XSWAPLP  STA STACK,X
         ...
         JMP **         ADDRESS FOR JUMP IS CHANGED ABOVE
;
...
;
        LDA PAGE0       DO WE NEED TO SWAP PAGES 0/l
...
; ANOTHRR EXAMPLE OF TURNING A TWO BYTE INSTRUCTION INTO A THREE
...
STROUT  PLA             PULL RETURN ADDRESS OFF STACK
        ...
        LDA (STROAD),Y  GET 8YTE FROM STRING
...
;
PRTHEX  AND #S0F        MASK UPPER 4 BITS
...
;
; ONE OF TilE MOST USEFUL ROUTINES IN THE OS IF YOU ARE WRITTING
...
NXTDS   LDA TSI         RESTORE BUFFER OFFSET
        STA BUFOFS
...
; PURPOSE.
;
        SWAP0 = *+256   PAGE 9 HOLD AREA (USED BY BASIC)
        SWAP1 = *+256   STACK  "    "     "    "  "
;
