...
PTRPIA  =   $FD00       PARALLEL PRINTER PIA (MC682l)
...
... ND IS SET BY EACH INDIVIDUAL ROUTINE BEFORE CALLING '£HIS ROUTINE.
;
2395 80 63 23 COM INC
2398 FE 8.0. 23
239B 00 "3
2390 FE 8B 23
23.0.11 60
...
($23.0.0)
;
; DKIIN : DISK 1 INPUT ROUTINE (BASIC DEVICE 6)
;
23.0.1 A~
23.0.3 211
23.0.6 00
...
BNE COMINC
SET Y OFFSET
CHECK YOR END OF BUFFER
($23AB) IF NOT END OF BUFFER, CONT
READ NEXT TRACK
LOAD BYTE (MODIFIED BY COMINC)
SET THE OFFSET
GO USE COMMON INCREMENT HOUTINE
;
)
; DKI0UT : DISK 1 OUTPUT ROUTINE (BASIC DEVICE 6)
;
THIS ROUTINE WILL ALLOW YOU TO PRINT ANY CHARACTERS TO DISK EXCEPT
; A LINE YEED ($0.0.). SOMETIMES IT IS USEFUL TO BE ABLE TO WRITE A
; LINE FEED TO DISK, I.R. CREATING A WORD PROCESSOR OR ASSEMBLER
; FILE WITH BASIC. IF YOU WISH TO DO SO, YOU CAN CHANGE THE FOURTH
BYTE OF EITHER DISK OUTPUT ROUTINE TO ... NULL (HEX U). JUST BE SURE
...
23CA 00 C9
BNE COMIt~C
If LINE .'£1::0 THEN IlETUHN
($23A0)
SAVE BYTE TO BE WRI'£TEN
SET Y FOR OFFSET
CHECK FOR END OY BUFFER
($23Cl) CONTINUE IF NOT AT END
WRITE THIS TRACK, READ NEXT
RESTORE THE OUTPUT BYTE
PUT IN BUFFER (MODIFIED BY COMINC)
SET OFFSET FOR COMMON INCREMENT
SET BUFFER DIRTY FLAG
BRANCH TO COMMON INCREMENT
;
; DKINXT : DISK 1 NEXT TRACK READ, USED BY DKIIN AND DKIOUT
,
)
23CC AD 20 23 DKINXT LDA DIBFDR
23CF FII 115
IlEQ *+7
LOX 'J$0C
2301 A2 00
GET IlUFFER 'DIRTY' FLAG
($2306) IF NOT 'DIRTY' CONTINUE
SET O.' FSETOS65D V3.2 DISASSEMBLY
PAGE: 12
--------------- -- ----- --- - ---------------------------------------------
)
23D3 20 77 24
...
SET OFFSETS
CHECK FOR END OF BUFFER
($2414) IF NOT END THEN CONTINUE
WRITE BUFFER, READ NEXT TRACK
GET BYTE TO BE WRITTEN
PUT IN BUFFER (MODIFIED BY COMINC)
SET OFFSET FOR COMINC
SET BUFFER 'DIRTY' FLAG
...
2420 AD 35 23 OK2NXT LDA D2BFDR
2423 FIl 05
BEQ *+7
2425 A2 08
LOX '$08
...
2438 8D FE 23
S'fA D2 IADR+ 1
GET BUFFER 'DIRTY' FLAG
($242A) CONTINUE IF NOT 'DIRTY'
SET OFFSET
GO WRITE THIS BUFFER
RESET READ/WRITE ADDRESSES
AND MEMORY ADDRESS TO
STAIlT OF BUFFER
...
GET SET TO ADD 1 TO CURRENT TRACK
SET DECIMAL (TRACK. IN BCD)
DO THE ADD
CLEAR DECIMAL MODE
SAVE THE TRACK NUMBER
MOVE HEAD TO TRACK
($2BID) READ DISK, UllLOAD HEAD
AND RETURN
;
;
BD~IHTK
: BUFFERED DISK I/O MOVE HEAD TO TRACK
;
)
2453 A9 00
BDMHTK LDA 1$00
...
WITH LAST TRACK
($2C8D) MOVE HEAD TO 'rtlACK, IF
PAST END OF FILE, ERROR 0
SET Y TO 1
ALWAYS BRANCH TO PATCH2
(NOT USED)
(NOT USED)
;
2466 A2 nil
...
CMP D1BFHI+l,X
RTS
LOW ADDRESS OF BYTE TO BE READ
LOW ADDRESS OF END OF BUFf'EIl
($2476) IF NOT THE SAME THEN RETURN
HI ADDRESS OF BYTE TO BE READ
HI ADDRESS OF END OF BUFFER
RETURN WITH Z FLAG SET IF END
...
STA MEMLO
\
HI ADDR OF BUFFER HI ADDR
HI ADDR OF BUFFER LOW ADDR
NUMBER OF PAGES
SET MEMORY ADDRESS TO LOI'
BUFFER ADDRESSOS65D V3.2 DISASSEMBLY
PAGE: 14
-----------------------------------------------------------------------
)
2486 ~D
...
JMP DSKWHT
MOVE HEAD TO TRACK
WHITI:: TO DISK AND HETURN
(FROM $2462)
;
2491 8C 5E 26 PATCH2 S·fY SECTNM
2494 4C 54 27
JMP LDHEAD
...
,·
; PTHOUT : PARALLEL PRINTER OUTPUT DEVICE (BASIC DEVICE 4)
;
)
;
;
...
,
NOTE:
SOME OF THE NEWER PRINTERS ON THE MARKET ARE EQUIPPED \O/ITH
GRAPHICS AND NEED THE ~'ULL 8 BITS OF AN OUTPUT ~YTE TO USF.
THIS FEATURE. CHANGING 'fHE INS1'RUC'rrON AT $24A7 AND $24A8 TO
...
24AF 61l
RTS
SAVE ~YTE TO BE PRINTED
CHECK PIA STATUS REGISTER
($24A0) NOT CLEAR, KEEP WAITING
RESTORE THE OUTPUT BYTE
KILL THE UPPER BIT
OUTPU'f'THE BYTE
STROBE 'fiu: ~Yn; 'fO THE PRINTEH
;
; BEFORE USING EI'fHt;H OF 'fHE CA10X ROUTINES, THE PORT NUMBER MUST
; BE SET IN X16DEV ($2323)
;
; X16lNP : CA10X INPUT ROUTINE (BASIC DEVICE 8)
;
24B0 AE 23 23 X16INP
~D Illl CF
24B6 4A
24B7 90 F7
24B9 B0 4D
...
BRK
GET ACIA DEVICE.
GET ACIA STA'fUS REG ISTER
SHIFT STATUS BIT TO CARRY
TRY AGAIN IF NOT READY
($2508) GO 'fO PATCH3 TO INPUT
(NOT USED)
(NOT USED)
; X160UT : CA10X OUTPUT HOUTINI:: (BASIC DEVICE 8)I
PAGE: 15
...
23
RTS
SAVE THE BY'fE TO OUTPUT
GET THE ACIA STATUS
FC
($24CE) IF NOT READY, TRY AGAIN
...
CONTROL Q7
($24EA) NO, TRY AGAIN
RESTORE THE OUTPUT BY1'g
SAVE IT
,
; TERMIN : SERIAL TERMINAL INPUT ROUTINE (BASIC
OEVIC ~
1)
,
24F6 AD 00 FC TERMIN LOA TERMAC
INC RNDSED
24F9 EE 24 23
LSR A
2H'C 4A
...
AND '$7F
2502 29 7F
2504 80 63 2) TlilTN STA A.HOLD
RTS
2507 60
GET ACIA STATUS
...
SAVE TliE CHARACTER
;
; PATCH) : ADDED TO X16INP ROUTINE
(FROM $24B9)
,
2508 BO 01 CF PATCH) LOA X16ACI+l,X
BCS TIRTN
2 5"B B~ F7
);
GI::1' BYTE ,'ROM ACIA
PU '!' IN A. HOLD AND RI::TUHN
...
STA UAR1'+4
RTS
SAVE '!'HI:: BYTE TO OUTPUT
GE'r UART STATUS
($250E) NOT READY, TRY AGAI '~
RESTORE THE OUTPUT CHARACTER
AND OUTPu'r I'r
;
;
430 BOARD UART INPUT (BASIC DEVICE 3)
SERINP
;
2518
251B
...
POLLED KEYBOAIW INPUT ROUTINE (BASIC DEVICE 2)
; THIS ROUTINg USES THE SAME ROUTINE AS THE ROM BASED MACHINES.
UNFORTUNATELY, THE DISK BASIC USES SOME OF THE SAME MEMORY
LOCATIONS AS THE ROUTI NE AT $FD00. INSTEAD OF DOING THE CORREc'r
...
KBINP+3
SAVE $213-$216
BUMP THE RANDOM SE~D
CALL THE ROUTINE IN ROM
($252E) IF NULL THEN TRY AGAIN
THI S IS ANOTH~R STHANGE INSTRUCTION. THE PRESEN1' KgYB OARD ROU'fINt:
WAI'!':; UNTIL A KEY IS PRgSSED AND THEN RETURNS IT'S ASCII VALUE.
A NULL IS NEVER RETURNED FROM THE PRESENT KEYBOARD ROUTINE SO
IT MAKES NO SENSE TO CHECK ~'OR I'r.
;
2536 BD 63 23
2539 2fJ 44 26
253C AD 63 23
...
LDA A.HOLD
RTS
SAVE THE INPUT CHARAC1'ER
RESTORE $213-$ 2 16
GET THE INPUT CHARACTER
USED BY 5 4 0 VIDE O DRIVER FOR KEY PRESSED DURING OUTPUT
PATCH 4
(FRON $25~' 2)
;
)
2540 8D 25 23 PATCH4 STA KPDO
2543 68
PLA
2544 4C 04 25
JMP TIIITN
SAVE THE CHARACTER
SAVE A AND RETURN05650 V3.2 DISASSEMBLY
PAGE: 17
-----------------------------------------------------------------------
I
i
...
; WHEN YOU EXIT 'ro THE MONITOR AND THEN RE-ENTER THE OS AT S2A51, YOU
; WILL NORMALLY HAVE PROBLEMS WITH THE POLLEfr· KEYBOARD ROUTINE. BY
; ENTERING AT S2547, THE 4 BYTES FROM S9213-S0216 ARE RESTORED AND THE
I KEYBOARD ROUTINE WILL WORK CORRECTLY.
,·
JSR SWAP4
JMP OS65D3
2547 211 44 26
...
·
,·
I THE CONTROL P IS A NICE
,
SWAP THE 4 BYTES
JUMP TO THE OS
; CKINP : CHECK INPUT FOR INDIRECT FILE COMMANDS AND CONTROL P.
F~ATURE THAT WE HAVE NEVER SEEN DOCUMENTED
; BY OSI. UNDER VERSION 3.11 IT WAS A CONTROL T, WHILE UNDER
; VERSION , 3.2 I.T HAS BEEN CHANGED TO ..A CONTROL p. TJlIS C.ONTROL
; CHA'RACTER, .WHICHEVER ONE IT IS, FLIP,-FLOPS A FLAG THAT CONTROLS '
I PRiN'rER OUTPUT.: THE .. ~'IRST TIME THE CONTROL CHARACTER IS I::NCOUNTERED
I IT TURNS ON THE PRINTI::R DEVICE AND THE NEXT TIME IT TURNS IT OFF.
I WARNINGI SOME OF THE SOFTWARE PROVIDED ON THE SYST~M USES THIS
...
; DOES NOT AFFECT l'r AND IT , REMAINS ON UNTIL 'rHERE IS ANOTHER
...
254F Oil 11
CKINP
(I) START INDIRECT FILE?
NO, CONTINUE
SET UPPER ADDRESS FOR INDIRECT
MODIFY MEMORY OUTPUT ROUTINE
SET LOWER ADDRESS FOR INDIRECT
MODIFY MEMORY OUTPUT ROUTINE
SET MEMORY OUTPUT
ALWAYS BRANCH TO EXIT
(I) CLOSE INDIRECT FILE?
NO, CONTINUE
PRINT 'J', BYPASS SAVAXY
I/O DEFAULT DEVICE
RESET INPUT POINTER
GET THE PRESENT OUTPUT DEVICE(S)
TURN OFF MEMORY OUTPUT
SAVE TilE' OUTPu'r DISTRIBUTOR
PUT 'J' BACK IN A
CONTROL X? (LOAD INDIRt::CT t'ILt:)
NO, CONTINUE
SET FOR ~IEMOI(,{ INPUT
GOSUB TO SET INPUT HIGH ADDRESS
SET INPUT LOW ADDRESS
($2596) ALWAYS BRANCH TO EXIT
IS I'f CONTROL P
($2598) NO, JU/~P TO Exn'
GET THI:: OUTPUT DISTRIBUTOR
- ,-
"" -
----OS65D V3.2 uISASSEMBLY
2591
2593
2596
2598
49 08
EOR IS08
UD 22 23 CKIR'fN STA OUTDST
A9 !HI
LDA 'S01l
RTS
60
PAGE: 18
FLIP-FLOP THE PRINTER OUTPUT
SAVE THE DISTRIBUTOR
DENOTES CONTROL CHARACT£H FOUNI)
...
AND FORWARD SPACE. CONSIDERING THE SOFT\~ARE SUPPLIED WITH OTHER
...
COULD MAKE WOULD BE TO CHANGE 'fHE INSTRUCTIONS FROM $25B9 TO
$25CB AND $25Al,$25A2 TO NOP'S. THIS WILL ALLOW YOU TO PRINT SOME
GRAPHICS CHARACTERS. WARNINGI THIS ROUTINE IS BAD ABOUT USING
SELF MODIFYING CODE.
;
)
VIDOUT TYA
...
GET OUPUT CHARACTER
STRIP TO 7 BIT ASCII
GET OFFSET IN PRINT LINE
I S IT A 'CR'
YES, DO IT
IF IT A 'LF'
YES, DO IT
BACKSPACE? ( non dest cut H)
YES, DO IT
IS IT CNTRL P
YES, DO IT
IS IT CNTllL L ( forwa rd space non dest)
YES, DO IT
IS IT < : SPACE'
YES, INVALID CHARACTER
IS IT > • ( •
YES, INVALID CHARACTER
OUTPUT CHARACTER TO SCREEN
BUMP LINE POINTER
LAST CHAllACTEH ON LINE
YES, ])0 SCROLL
GET CHAR. 'UNDEH' NEW CURSOR
SAVE IT
GET CURSOR CHAHACTER
OUTPUT IT
SAVE OFFSET
HESTORE Y
CHECK FOH 'CNTHL'
...
AND SAVE
INPUT FROM POLLED KEYBOARD
CNTRL S?
($25EB) YES, KEEP LOOPING
EXIT THE ROUTINE
RESTORE CHAR. 'UNDER' CURSOR
PRINT IT
BUMP LINE POINTER BACK 1
GO BACK
RESTORE CHAR. 'UNDER' CURSOR
BUMP LINE POINTER
EXIT THIS ROUTINE
RESTORE CIIAR. 'UNDER' CURSOR
RESET LINE POINTER
JUMP TO EXIT
RESET LINE POINTER
($2613) JUMP A LITTLE
RESTORE CHAR. 'UNDER' CURSOR
SAVE LINE OFFSET
SET TO CLEAR LOWER LINE
SE'r OFFSET
OUTPUT IT
BUMP THE OFFSET
($261A) LOOP UNTIL DONE
GET SET TO SCROLL
FIRST TIME THROUGH Y = $D0
ADJUS'r LINE POINTER
ADJUST LINE POINTER
MOVE UP 1 LINE AT A TIME
BUMP THE LINE POINTER
IF MOVED LINE, SET FOR NEXT
KEEP LOOPING
HAVE \~E DONE 'tHEM ALL
NO, KEEp· LOOPING
RESTORE LINE OFFSET
JUMP TO EXIT
...
i
I{ EYT~T
'T'F:ST POLLED KEYBOAHD FOR KEYDOWN IN ROW IN
ACCU~I
;
)
263D 8D g(l DF KEYTST STA KPOR't
BIT KPORT
264H 2C ~0 DF
KTHTN
IlT5
...
CHECK FOH KEY DOWN
; SWAP4
PATCI! ADDED TO ENABLE USE OF POLLED KEYBOARD 1l0UTINE
; @$FDg0. SWAPS OUT 4 BYTES fHOM 5213-$216 TO $2657-$265AOS6 50 V3. 2 DI SASSEM BLY
)
;
sWAP4
2644 A2 "3
...
RTs
sWAP4B = *+4
SE'r INDEX FOR 4 BYTE S
SWAP A BYTE
($2646)
IF ANOTHER . CONTINUE
SWAP AREA FOH $D21 3- S021 6OS65D V3.2
)
PAGE: 21
OISASS~MBLY
...
='
TRKNM ='
(UNKNOWN USAGE, IF ANY)
PRESENT DISK DRIVE
CURRENT TRACK NUMBER
PRESENT SECTOR NUMBER
PAGE COUNT
LOW ADDRESS OF MEMORY BLOCK
HI ADDRESS OF MEMORY BLOCK
HEX TRACK NUMBER
NOT USED, SEE NOTE @ $26A6
; HOME : HOMES HEAD TO 'fHACK \I ON CURRENT DISK DRIVE
;
JSR STEPOT
2663 20 SA 26 HOME
JSR TENMS
...
SET TRACK' TO 0
CHECK FOR TRACK 0
DELAY 10MS AND HETUllN IF 1'}{
STEP HEAD IN
LOOP BACK AND THY AGAIN
~
;
; TENMS : 10 MS DELAY. ACTUALLY @ IMHZ THE DELAY IS CLOSER TO 11 MS
,,",. -1 ..
)
2678 A2 0C
TENMS
...
ON
;
; STEP IN : STEP 'fOWARDS 'I'HACK Il
; MOVES !lEAD ONE TRACK
,
2683 AD 02 C0 STEPIN LDA FLOPOT
2686 09 04
ORA #$1J4
2688 D0 05
BNE STEP
...
($2706) KILL 14 CYCLES
TURN ON STEP tlI'f
STA @ $C~02 AND IlETUHN
GET STEP RATE
(S267A) DELAY S'l'EP RATE NS
'~unNOS65D V3.2 DISASSEMBLY
PAGE: 22
) -----------------------------------------------------------------------
;
; (ROUTINE @ S26A6)
THIS ROUTINE CONVERTS A HEX TRACK NUMBEH
; AT S2662 TO BCD AND STORES IT AT SEE, THEN FALLS INTO THE SET
; TRACK ROUTINE. THE HOUTINE IS NOT USED BY THE OS, SO EITHEH
...
TKNHLD
26BA 05 EE
IN IT X TO COUNT 10'S
SUBTRACT 10 FROM TRACK'
(S26AC) IF >=0 BUMP X AND DO AGAIN
ADD BACK LAST 10 FOR REMAINDER
SAVE REMAINDER
GET NUMBER OF TENS
SHIFT TO HIGH NIBBLE
COMBINE WITH REMAINDER
•
; SETTK : CHECK f'OI{ VALID 'fRACK NUMBER AND MOVE HEAD THEHE
...
ERR8
BNE ERR5+2
(S26DE) JUMP TO ERROR HANDLER
26CF DO 0D
26Dl AD 5C 26 MOVEHD LDA DSKDR
GET DISK DRIVE
AND f$el
TOP DRIVE=I, BOTTOM DRIVE='"
26D4 29 01
TAY
26D6 AS
JSR CKRDY
...
CKTK
CMP 'rIINUM
SAME AS PRESENT TRACK NUMBER?
26E3 CD 5D 26
BEQ STCCN'f (f'!7~) YES, DON'T MOVE THE HEAD
26,1::6 F0 20
BCS *+9
(S26Fl) BRANCH IF > PRESENT TRACK
26E8 B0 07
JSR STEPIN
...
ADC TKNUM
26FG 6D 50 26
ADD OR SUBTRACT 1 FROM l'KNU~1OS55D V3.2 DISASSEMBLY
PAGE: 23
) -----------------------------------------------------------------------
STA TKNUM
CLD
...
ORA F['OPOT
2716 0D 02 CII
2719 8D 02 C0 SETFLO S'rA f['OPOT
RTS
;71C 60
; WAITIH
ARE l'iE PAST TRACK 42
RESET LOW CURRENT BIT
WHO KNOWS?
IF PAST TRACK 42, CONTINUE
(PIA2) SET ['oW CURRENT BIT
STORE I'r
WAIT FOR INDEX HOLE
271D AD 00 CO WAITIH LDA FLOPIN
BMI l'iAI'l'IIi
2720 30 FB
LDA fLOPIN
2722 AD 00 CIl
...
2727 60
GET DISK STATUS
If BIT 7 ON, GO TEST AGAIN
GET DISK STATUS
($2722) IF BIT 7 OFF, TRY AGAIN
,
; LDHDWI
['OAD HEAD AND WAIT
~'O R
...
;
LOAD HEAD
2728 20 54 27 LDHDWI JSR ['DHEAD
;
; RSACIA : RESET DISK ACIA, WAIT
~' OR
INDEX HOLE
;
272B 29 ' ID 27 RSACIA JSR WAITIH
272E A9 03
['oA 'S03
2730 80 10 C0
STA ACIA
...
2738 60
RTS
WAIT FOR , THE INDEX 1I0LE
MASTER RESET fOR ACIA
SET FOR /1, RTS=l, NO INTERUPT
;
; EXAMCN
,
)
...
LOAD HEAD, WAIT FOR INDEX HOLE
GET 'fliE STATUS
IF AT INDEX HOLt:, UN['OA IJ HEAD
GET ACIA STATUSOS65D V3.2
)
2745 90
2747 AD
274A 91
...
UISASS~MBLY
PAGE: 24
($273C) NOT READY, WAIT fOR INDEX
READ A BYTE
STORE IT IN MEMORY
11'1'1
($273C) IF MORE IN THIS PAGE
·BUMP MEMORY ADDRESS
($273C) CONTINUE
•
...
DO ERROR #4
JUMP TO ERROR HANDLER
LOAD HEAD AND WAIT FOH INDEX
GET SET 'ro TURN ON
WRITE ENABU: AND ERASE ENABLE
DO
1 ms DELAY
'rRACK STAHT COllE llYTl::lOS6SD V3.2 DISASSEMBLY
PAGE: 25
-----------------------------------------------------------------------
)
27A6 20 C2 27
